  ...
  setUp(() {
    ...
    provideDummy<DataResult<MusclesByIdResponseEntity>>(
      Success(const MusclesByIdResponseEntity(
          muscles: [],
          muscleGroup: MuscleGroupEntityById(id: '', name: ''),
          message: '')),
    );
  });

  group('when calling getAllWorkouts, it should call WorkoutsOnlineDataSource',
      () {
    test(
    ...
    test(
        'when calling getAllWorkouts, it should fetch data from WorkoutsOnlineDataSource.getWorkoutsById',
        () async {
      ...
      when(mockWorkoutsOnlineDataSource.getWorkoutById('1')).thenAnswer(
        (_) async =>
            Success<MusclesByIdResponseEntity>(musclesByIdResponseEntity),
      );
      var result = await workoutsRepoImpl.getWorkoutById(
        '1',
      );
